...
class DashboardPage(BasePage):
    ...
    def check_visible_students_chart(self):
        expect(self.students_title).to_have_visible()
        expect(self.students_title).to_have_text('Students')
        expect(self.students_chart).to_have_visible()

    def check_visible_activities_chart(self):
        expect(self.activities_title).to_have_visible()
        expect(self.activities_title).to_have_text('Activities')
        expect(self.activities_chart).to_have_visible()
    def check_visible_scores_chart(self):
        expect(self.scores_title).to_have_visible()
        expect(self.scores_title).to_have_text('Scores')
        expect(self.scores_chart).to_have_visible()

    def check_visible_courses_chart(self):
        expect(self.courses_title).to_have_visible()
        expect(self.courses_title).to_have_text('Courses')
        expect(self.courses_chart).to_have_visible()
